...
TARGET_FILE="/misskey/packages/backend/built/boot/entry.js"
PATCH_OLD='function AJ(n){return{date:PJ(n.slice(0,10)),additional:sJ(n.slice(10,26))}}'
PATCH_NEW='function AJ(n){let e=0n,t=n.slice(10,26);for(let r=0;r<t.length;r++)e=e*32n+BigInt(rMe.indexOf(t[r]));return{date:PJ(n.slice(0,10)),additional:e}}'
...
tmpdir="$(mktemp -d)"
local_file="$tmpdir/entry.js"
backup_file="/tmp/entry.js.bak.$(date +%Y%m%d-%H%M%S)"
